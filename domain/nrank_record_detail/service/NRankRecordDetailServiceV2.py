...
async def get_current_page_response(page_index, create_req_dto):
    params = {
        'frm': 'NVSHTTL',
...
async def search_page_and_get_rank_models(page_index, create_req_dto):
    # get response by naver ranking page
    searchResponse = await asyncio.create_task(get_current_page_response(page_index, create_req_dto))
...
def updateRankForAdProduct(create_req_dto, results):
...
async def request_shopping_ranking(create_req_dto):
    ranking_results = []
    # pageSize 만큼 비동기 요청
    rank_entities = [search_page_and_get_rank_models(i+1, create_req_dto) for i in range(create_req_dto.page_size)]
...
def create_nrank_record_info(record_info, results):
    nrank_record_info_repository = NRankRecordInfoRepository()
